...
/**
 * <p>SubscriberAwareListener</p>
 *
 * <p>创建于 2024/2/9 19:08</p>
 *
 * @author ketikai
 * @version 1.0.0
 * @since 1.0.0
 */
public class SubscriberAwareListener implements Listener {

    private final ContentPublisher contentPublisher;

    public SubscriberAwareListener(@NotNull ContentPublisher contentPublisher) {
    ...
    public void on(PluginEnableEvent event) {
        if (event.getPlugin() instanceof SubscriberAware) {
            ((SubscriberAware) event.getPlugin()).setSubscriber(contentPublisher::subscribe);
        }
    }
}
